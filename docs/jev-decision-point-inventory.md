...
4. **Budget**（§3/§6-4）: 予算は**fixture件数そのものではなく、実際に
   ...
   単純合計ではない。現在、実際にJevへ送信可能な件数はDP-4が6件
   （3件を非公開情報のため、1件（旧DP4-09）を実行主体が公開裏付けで
   検証できないため、計4件削除済み、§8.1.1参照）、**DP-9は0件**
   ...
   参照）、合計**6件**である。これで計算すると、初回分6回＋
   Reproducibility分`6×3=18`回で**最低24回**（さらにネットワーク/
