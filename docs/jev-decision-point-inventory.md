...
4. **Budget**（§3/§6-4）: 予算は**fixture件数そのものではなく、実際に
   ...
   単純合計ではない。**さらに、fixtureあたりの初回リクエスト数は
   DP種別で異なり一律ではない（今回のCodex指摘への対応として明記）**：
   DP-4（Choice）とDP-10（Noul）はfixtureごとに単一のtyped questionしか
   送らないため、初回1回＋Reproducibility 3回＝**fixtureあたり4回**。
   一方DP-9は§8.2.3が定める通りChoiceとScoreを**2つの独立したtyped
   question**としてそれぞれ送るため、fixtureごとに初回2回（Choice 1回＋
   Score 1回）＋Reproducibility 2×3=6回＝**fixtureあたり8回**であり、
   DP-4/DP-10と同じ「fixture数×4」を適用すると過小算出になる。したがって
   総リクエスト数は`DP-4件数×4 + DP-9件数×8 + DP-10件数×4`で計算する
   （この式自体が予算算出の正本であり、以後のfixture件数変動時も
   この式へ最新件数を代入して再計算する）。現在、実際にJevへ送信可能な
   件数はDP-4が6件（3件を非公開情報のため、1件（旧DP4-09）を実行主体が
   公開裏付けで検証できないため、計4件削除済み、§8.1.1参照）、**DP-9は
   0件**（今回のCodex指摘への対応として明記。データセット自体は10件
   ...
   参照）である。上記の式に代入すると、DP-4分`6×4=24`回＋DP-9分
   `0×8=0`回＋DP-10分`0×4=0`回で**最低24回**（さらにネットワーク/
   レート制限起因の再試行を見込むとこれを上回る）——DP-9が現時点で
   0件のため今回の合計値そのものはfixture数×4で計算した場合と一致
   するが、**DP-9側でrequest schemaを完全に満たす新規fixtureが確保
   できた時点で「fixture数×4」ではなく必ず上記の`DP-9件数×8`を含む
   式で再計算すること。** **この件数を予算の上限だと誤解してはならない
   ——T04は実行直前に、その時点のfixture件数・再送回数・想定リトライ数
   から総リクエスト数を上記の式で計算し直し、その数値をNotion Task
   （`ADP-065-T04`）へ明記した上で送信を開始する。** §8.4のギャップ
   解消でfixture件数が変わった場合、またはDP-9/DP-10側でrequest
   schemaを完全に満たす新規fixtureが確保できた場合も同様に再計算する。
   この総数を超える追加送信（新規fixtureの追加や再々試行を含む）は、
...
- **変更する場合**: 0.7を変更する必要が生じた場合は、評価対象
  フィクスチャ（DP-4は6件、DP-9は10件、DP-10は現時点で0件——本書収録
...
**DP4-09（欠番）について（今回のCodexレビューへの対応。旧行を完全に削除）**:
旧DP4-09（`cloud42-labo/experimental`リポジトリvisibility変更、
Private→Public）は、複数回の修正を経て`actor`列を`Human`で凍結して
いたが、その根拠は「GitHubの一般的なアクセス制御モデル上、リポジトリ
visibility変更はowner/admin権限のアカウントでのみ実行可能」という
一般論に過ぎず、**この行為を実際に実行した主体がHumanであることそのもの
を裏付ける公開証跡には到達していなかった**。適切にスコープされた
個人アクセストークンを使えばAI/自動化がAPI経由で同じ変更を実行できる
可能性を排除できておらず、リポジトリが現にPublicであるという事実は
「変更が発生したこと」を示すのみで「誰が実行したか」を示さない。
`actor`は本書のrequest schema（§8.1.3）の直接入力でありモデル出力に
影響するため、未検証の値のまま凍結し続けることはできない。

本セッションはGitHubの公開Events API（`/repos/cloud42-labo/experimental/
events`）を確認したが、保持されているイベント履歴は直近300件・
2026-08-30以降分までしか遡れず（`experimental`はリポジトリ作成が
2026-07-25）、visibility変更時に発生する`PublicEvent`は現在の公開API
レスポンスに含まれていなかった。GitHubは非管理者向けにリポジトリ設定
変更の監査ログを公開APIで提供していないため、これ以上の公開手段による
実行主体の検証は見込めないと判断した。したがって旧DP4-04・旧DP4-05・
旧DP4-06と同じ「検証不能なフィクスチャは残さない」方針に従い、
Pre-decision input／environment・authority note／Ground truthの3表から
旧DP4-09の行を出典・識別情報を含めて完全に削除した。番号`DP4-09`は
振り直さず欠番として扱う。今後、GitHub組織の監査ログ（Admin/Audit log
API）またはOwner本人への直接確認等、非公開ソースに頼らずに実行主体を
検証できるアクセスを持つセッションが、代替フィクスチャを再導出するか、
新規番号（`DP4-11`以降）として追加することを推奨する。
...
扱う。上記の通り旧DP4-09も実行主体が公開裏付けで検証できず同じ扱いで
削除したため、欠番は`DP4-04`・`DP4-05`・`DP4-06`・`DP4-09`の4つとなり、
残る6件（DP4-01, 02, 03, 07, 08, 10）が元の番号のまま維持される。
DP-4のfixture数は10件から**6件**（客観4件：DP4-01, 02, 03, 10／
曖昧境界2件：DP4-07, 08）へ変更し、§8.0.1のBudget再計算・
§8.1.4のAccuracy/Calibration/escalation-rate分母・§8.4のギャップ要約へ
反映済み（各該当箇所参照）。

なお、削除しなかった残り6件については、それぞれ独立した公開裏付け
（本セッション自身のGitHub操作、対応するPR/commitの現存、対応する
リポジトリの現在の公開設定等、上表の出典列が示す情報）を本セッションが
直接確認した上で維持している。

今後、Notion/`brain`アクセスを持つセッションが、公開裏付けのある
代替フィクスチャを再導出し、削除した4枠（またはそれに代わる新規番号
...
1. **Threshold sanity check（閾値は既に確定済み、ここで決めない）**:
   ...
   評価対象の6件全件とは独立したholdoutキャリブレーションセットを
   別途用意し、評価出力を見る前に較正を完了させた上で変更する（§8.0.2）。
2. **Accuracy（客観4件）とAgreement（曖昧境界2件）を別指標として算出・
   報告する**: 6件全件をJevへ送り、`choice`と§8.1.2のground truthを
   完全一致（exact match）で比較する。ただし見出しの**Accuracy**は
   境界が明確な客観4件（DP4-01, 02, 03, 10）のみを分母とし、
   `Accuracy = 客観4件中の一致数 / 4`として計算する。曖昧境界2件
   （DP4-07, 08）はground truth自体が「現行運用解釈」であり
   客観的な正解ではないため、この4件のAccuracyには一切混ぜない
   （不一致を分子側で除外するのではなく、そもそも分母から外す）。
   曖昧境界2件については、Jevの出力と現行運用解釈が一致したかを
   **Agreement（曖昧境界2件、定性記述）**として別途報告する——
...
3. **Calibration**: **客観4件（DP4-01, 02, 03, 10）のみを対象に
   ...
   なのに現行運用解釈と相違）が境界2件に集中していないかは、この別掲の
   confidence記録を見て確認する。
4. **Latency**: 6件個別呼び出しのwall-clock時間をp50/p95で記録。
5. **Cost（今回のCodex指摘への対応として、per-passとfull-experimentを
   分離・併記する）**: outputは無料のため、input tokens
   （state+question長）×$0.042/MTokのみで計算する。
   - **Per-pass cost**: このDPの初回送信分（DP-4は6件、DP-9は現時点で
   ...
   - **Full-experiment cost（§8.0.1のBudget修正と対応させる。今回の
     ...
     **Full-experiment costは「初回件数×4」という固定式ではなく、
     ...
     （現在の件数なら最低24回、これも実行前の下限見積もりであり実際の
...
7. **Missed-escalation rate**: §8.1.3の「false-escalation /
   ...
   expected-escalate実例数」（曖昧境界2件を除く客観4件のうち、期待
   ...
   曖昧境界2件（DP4-07, 08）はこの分母・分子のいずれにも含めない
   ——step 3のCalibration除外と同じ理由（ground truthが客観的な正解では
   なく現行運用解釈であるため）。曖昧境界2件についてのescalation属性の
...
8. **False-escalation rate**: 同定義に従い、最終決定のescalation属性が
   ...
   **分母は「検証済みのexpected-no-escalate実例数」（曖昧境界2件を除く
   ...
   曖昧境界2件はここでも分母・分子から除外し、Agreement側で別途扱う
   ...
   誤るケース。本節の客観4件・曖昧境界2件には現在この具体例に該当する
...
§8.2.4 step 1・step 4）。**検証後の遷移先（今回のCodex指摘への対応と
して明記）**: 上記の独立した実行完了エビデンス（AC自体の事前見積もり
ではない、実測durationまたはTask Time Events等の客観記録）がstep 4で
確認され循環参照が解消された時点で、DP9-03は`fits-as-is`のground
truthが客観的に確立されたことになり、DP9-05〜10と同じ**Objective
Accuracy setへ算入する**（当初からcircularでなかった実例と同じ扱いに
昇格する）。ただし**Agreementへは算入しない**——DP9-03はDP-4の曖昧
境界例のようなraterごとの見解の相違を扱う実例ではなく、あくまで
「客観的なground truthが確立済みか未確立か」の問題だったため、
解消後の適切な区分はAgreementではなくAccuracyである。この遷移は
§8.2.4 step 4・§8.2.4 step 6（Accuracy/Agreementの区分手順）の両方に
反映する。
...
十分ではない。** 真に必要なのはTask全体のライフサイクル（着手〜完了判定）を
カバーするエビデンスであり、これは単一のソースだけでは揃わない
（詳細な定義・境界値は§8.2.4 step 5参照。以下はその要旨）。**Notion Task
Time Eventsの`Started At`→`Ended At`（`Σ Active Duration`）だけでは
不十分**——`integrations/notion-time-events/README.md:55`の通りTime
Eventは`Review`入りの時点でcloseするため、その後の独立レビュー・完了
判定に要した時間を捕捉しない。したがって、これに加えてTask自体の
`Started At`→`Completed At`のwall-clock差分（elapsed時間。Review・
完了判定を含む「着手〜完了」全体をカバーする）も別途必要であり、
`Completed At`は「TTEの終了時刻の代わりに使わない」ものではなく、
TTEが捉えない区間を補うために**必ず参照する**。主要指標へadmitするには
次の2つの独立エビデンスを両方満たす必要がある。

1. Task Time Eventsの`Σ Active Duration (h)`が8時間以下であること。
2. Taskの`Started At`→`Completed At`のelapsed時間も8時間以下で
   あること（Waiting区間を検証済みで控除できる場合はその控除後）。

本セッションはGitHub MCPツールのみに限定されており、いずれも取得できない。
...
4. **DP9-03の独立実行完了エビデンスの前提確認**: §8.2.2のDP9-03注記の
   ...
   **確認できた場合（今回のCodex指摘への対応として明記）**：DP9-03の
   `fits-as-is`ground truthは循環参照ではなく客観的に確立されたことに
   なるため、DP9-05〜10と同じ**Objective Accuracy setへ算入する**
   （Agreementへは算入しない——理由は§8.2.2のDP9-03注記の「検証後の
   遷移先」を参照）。以後の実行手順（step 6のAccuracy/Agreement区分、
   step 7のScore calibration admission判定等）でもDP9-03を通常の
   検証済みfits-as-is実例として扱う。
...
6. **Accuracy/Agreement（今回のCodex指摘への対応として新設。DP-4
   ...
   母集団はDP-4の「客観4件／曖昧境界2件」と同じ考え方で2つに分ける。
   ...
   - **Qualified/ambiguous ground truthの実例（DP9-04。ground truthが
     ...
     2件と同じ性質）**: Accuracyの分母・分子には一切混ぜない。代わりに
   ...
   - **循環参照ground truthの実例（DP9-03）**: DP-4の曖昧境界例とは異なり
     ...
     のsanity check相当の参考記録に留める。**循環性が解消された後の
     遷移先（今回のCodex指摘への対応として明記）**：step 4で独立実行
     完了エビデンスが確認された時点で、DP9-03のground truthはDP9-05〜10
     と同じ客観的なfits-as-is実例となるため、**Objective Accuracy setへ
     算入する（Agreementへは算入しない）**。詳細・理由は§8.2.2のDP9-03
     注記「検証後の遷移先」を参照。
...
7. **Calibration（今回のCodex指摘への対応として、`choice_confidence`と
   ...
   - **Score calibration**: `score_confidence`をbin化し、各binの
     ...
     用いると、この予測は誤って「正解」に分類され、reliability diagramの
     信頼性が過大評価される。**この点で、Score calibrationの正誤基準は
     §8.2.3の最終決定・false-escalation/missed-escalation rate（Score
     `== 2`かそれ以外かの2値のみで判定するescalationゲート、本節の
     定義は変更しない）よりも厳格である——両者は別の目的を持つ別の指標
     ...
     する：`Σ Active Duration`、および`Started At`→`Completed At`の
     elapsed時間（Waiting区間を検証済み控除した場合を含む）の**両方**
     が、その実例について**個別に検証済み**であること（＝step 5が
     DP9-05〜10のadmissionに要求するのと同じ独立duration評価エビデンス
     要件）。**ただし、step 5の`8時間`上限そのものはこのadmission
     ...
     **DP9-03は上記のいずれとも異なる**——step 4で独立実行完了
     ...
     従来通り。**現在の10件のfixture構成上の既知の限界（今回のCodex
     指摘を受け明記。§8.4とも整合）**：`fits-as-is`側（DP9-05〜10・
     DP9-03）はいずれもΣ Active Durationがstep 5の`8時間`以内に収まる
     ことを前提にラベル付けされた実例であり、`needs-split`側
     （DP9-01/02/04）は上記の通りScore calibrationから恒常的に除外
     される。したがって、8時間を超える検証済みdurationを持つ実例が
     別途追加されない限り、現行10件から実際にScore calibrationへ
     admitされ得るのはScore帯2の実例のみであり、常にScore 2を予測する
     モデルが見かけ上高いScore calibration正解率を示し得る。Score 3〜10
     帯の予測精度は、Notionアクセスを持つセッション等が1 AI working day
     を超える検証済みduration（`Started At`→`Completed At`の数値確認を
     含む）を持つ実例を追加収集するまで、実質的に未検証のままである
     （§8.4「実例の充足状況とギャップ」にも同じ限界を記載する）。
...
- **DP-4**: 当初10件確保していたが、うち3件（旧DP4-04・旧DP4-05・
  ...
  §8.1.1末尾の注記の通り行ごと削除した。さらに1件（旧DP4-09、
  `experimental`リポジトリvisibility変更）は、`actor`をHumanで凍結した
  根拠がGitHubの一般的な権限モデルという一般論にとどまり、実行主体
  そのものを裏付ける公開証跡（GitHub公開Events APIの`PublicEvent`等）
  に到達できなかったため、同じ基準で行ごと削除した（詳細は§8.1.1末尾の
  注記。削除の理由・経緯はそちらへ一本化し、本項では繰り返さない）。
  **現在は6件（客観4件＋曖昧境界2件）**。うち2件（DP4-07, 08）は境界が
  実際に曖昧な実例であり、`self-authority-escalation`カテゴリに該当する
  実インシデントは本セッションの検索範囲では発見できなかった（6件には
  含めていない）。残る6件は、それぞれ独立にPublic側の裏付けを本セッ
  ションが直接確認している（詳細は§8.1.1末尾の注記）。
- **DP-9**: 10件確保。`needs-split`側3件、`fits-as-is`側7件。**ただし
  ...
  - **Score calibrationの母集団構成上の既知の限界（今回のCodex指摘への
    対応として追記）**：上記の主要指標フィクスチャ不足とは別に、§8.2.4
    step 7のScore calibration admissionゲートは、検証済みduration
    エビデンスを持つ実例であればScore 3〜10帯も本来admit可能な設計へ
    修正したが、**現行10件のfixture構成では`fits-as-is`側（Score帯2
    相当）以外に検証対象となり得る実例が無いため、8時間超の検証済み
    durationを持つ実例が別途追加されるまで、Score calibrationは実質
    Score帯2の予測精度しか検証できない。** 常にScore 2を予測するモデル
    が見かけ上高いScore calibration正解率を示し得るという限界を、T04
    実行前に認識しておくこと。
