...
8. Task Sizing Failure関連KPIを次のとおり定義する。第12条のClosure semantics（`Closure Reason`／`Closed At`／`Completed At`）を正本とし、新たな計測の仕組みを追加しない。
   ...
   - **Refinement Effectiveness** = 分割後Task（`Split From` で元Taskを参照するTask群）の平均 `Lead Time (h)`、および**完了後**に発覚した品質実績（Done後のReopen回数、Human差し戻し・Rollback発生件数等のescaped defect）を、分割前の元Taskの実績と比較した差分。第14条3項によりDone移行時点のunresolved P0/P1は常にゼロであり、Done移行時点で測る指標は分割の効果を区別できないため、完了後に変動しうる指標を用いる（Codex Review, PR #54 第5ラウンド指摘）。Lead Timeの短縮、完了後の欠陥・差し戻しの悪化がないことを正の効果として記録する。同一領域の substantive review round数（`governance/review-loop-control.md` 第2節の定義）はTask sizing問題の診断Signalとして参照してよいが、同基準が明示するとおりReview回数そのものを成果指標にしない——round数の増減単独を効果の良し悪しとして記録しない（Codex Review, PR #54 第4ラウンド指摘）。
