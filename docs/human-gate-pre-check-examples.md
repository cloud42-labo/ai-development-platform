...
**Action:** if the criterion gates a downstream transition, do not block the current one on it — let the current transition proceed on its own actual gates, and route the Human-only work as its own request scoped to the transition it actually gates. The downstream gate itself is not removed, only prevented from propagating backward to a transition it does not govern.

Two worked regression cases — a criterion that turned out not to be a merge precondition at all, and a genuine downstream gate (real-environment deployment Acceptance) correctly kept for its own transition while wrongly blocking an earlier one — are in [`../governance/state-transition-pre-check-regression-cases.md`](../governance/state-transition-pre-check-regression-cases.md), drawn from Postmortem PM-8.
