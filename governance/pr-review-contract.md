...
- **When posting a review request** (`@codex review`, or relying on the PR's own open/ready-for-review event), the merge actor **must** record a collision-resistant digest of the exact current PR body being requested against (e.g. a SHA-256 of the body text) directly in the request comment, alongside its head SHA — not "where useful." Without a saved digest (or the full body text) captured *at request time*, a later `B1`→`B2` body edit leaves nothing to compare `B2` against at merge time except `B2` itself: the merge actor would see only the current body and have no way to establish whether it's still byte-identical to whatever `B1` was, silently letting a stale review through. This still requires no new external service, secret, or stored digest as a new *source of truth*: the body text is already read from the same GitHub API surface the SHA-based check uses, and the digest is a value mechanically derived from it, recorded where the request itself already lives (the PR comment thread).
