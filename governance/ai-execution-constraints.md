...
Trigger: immediately before creating a Stories & Tasks record with `Type = Human Request`, immediately before setting any task to `Blocked` for a reason that is Human action or Human confirmation, **and immediately before presenting or starting any existing Human Request with the Owner**. The gate must pass before that write or handoff, not after it. Existing metadata (`Type = Human Request`, `HUMAN-` title, `Assigned Agent = Human`, prior Approved Approach Review) is not proof that the work is still Human-only. Policy detail and classification guidance are in Operating Guide section 11.
