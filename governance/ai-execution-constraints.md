...
4. **Execution Event opened at genuine start** (`BUG-ADP-TTE-01-B`) — Active time must never be inferred purely from how long `Status` happens to read `In Progress`; the acting AI itself opens the record at the real moment substantive work begins. Create a page directly in the `Task Time Events` data source (the same data source `integrations/notion-time-events/Code.gs` writes to — see that integration's README, "Execution Event self-reporting") with `Task` (relation to this Task), `Actor` (select: `Claude`/`Codex`/`Chris`/`Human`, mapped the same way that integration maps `Assigned Agent`), `Started At` (the real wall-clock moment, not a value derived from polling), and `State = Active`. No special marker is required for this to be recognized: the reconciler treats any open (no `Ended At`) `Task Time Events` page for this Task/Actor as authoritative, regardless of whether it or the acting AI created it, so a self-opened event and the poller's own Status-driven bootstrap never conflict or duplicate — the existing same-actor dedup keeps exactly one.
5. **Execution Event closed at genuine stop** (`BUG-ADP-TTE-01-B`) — when the acting AI stops substantive execution (handing off, ending the session, or reaching a natural pause before `Status` itself changes), it closes the same event it opened by setting `Ended At` to the real wall-clock stop moment, rather than leaving it to be closed later, imprecisely, whenever `Status` next transitions away from `In Progress` and a poll happens to observe it. If `Status` transitions away from `In Progress` in the same action that ends the work (e.g. moving straight to `Review`), the poller's own transition handling closes the event and this step is not separately required.
