...
Trigger: immediately before creating a Stories & Tasks record with `Type = Human Request`, and immediately before setting any task to `Blocked` for a reason that is Human action or Human confirmation. The gate must pass before that write, not after it. Policy authority is defined by `docs/regulations/R02-authority-regulation.md` and `docs/regulations/R03-approval-regulation.md`; the classification and executable pre-check are defined here.
